...
class CompactBilinearPooling(nn.Module):
    ...
    def __init__(self, input_dim1, input_dim2, input_dim3, output_dim,
        ...
        if rand_h_3 is None:
            np.random.seed(9)
            rand_h_3 = np.random.randint(output_dim, size=self.input_dim3)
        if rand_s_3 is None:
            np.random.seed(11)
            rand_s_3 = 2 * np.random.randint(2, size=self.input_dim3) - 1

        self.sparse_sketch_matrix3 = Variable(self.generate_sketch_matrix(
            rand_h_3, rand_s_3, self.output_dim))
    ...
    def forward(self, bottom1, bottom2, bottom3):
        ...
        sketch_3 = bottom3_flat.mm(self.sparse_sketch_matrix3)
        ...
        fft3_real, fft3_imag = afft.Fft()(sketch_3, Variable(torch.zeros(sketch_3.size())).cuda())
        ...
        fft_product_real_new = fft_product_real.mul(fft3_real) - fft_product_imag.mul(fft3_imag) 
        fft_product_imag_new = fft_product_real.mul(fft3_imag) + fft_product_imag.mul(fft3_real)
